...
func main() {
	...
	queueType := flag.String("queue", "", "command type of queue (rabbitmq or aws)")
	...
	case "rabbitmq":
		...
		q, err = queue.NewRabbitMQQueue(ctx, *connectionString, *queueName, defaultBufferLength)
		if err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
	...
	// Initialize server
	s := server.NewServer(q, *maxWorkers)
